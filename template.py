...
def call_openai(
    ...
    return response.choices[0].message.content or "", max(latency, 1e-12)
...
def call_openai_mini(
    ...
    return call_openai(
        prompt, model=OPENAI_MINI_MODEL, temperature=temperature,
...
def compare_models(prompt: str) -> dict:
    ...
    output_tokens_estimate = len(gpt4o_response.split()) / 0.75
    cost = output_tokens_estimate / 1000 * PRICING_PER_1K_TOKENS["gpt-4o"]["output"]
    return {
...
def run_assistant(
    ...
    while max_turns is None or num_turns < max_turns:
        ...
        history = history[-6:]
        turn_cost = estimate_cost(user_msg, reply, model=OPENAI_MODEL)
        total_tokens += turn_cost["input_tokens"] + turn_cost["output_tokens"]
...
def batch_compare(prompts: list[str]) -> list[dict]:
    ...
    for prompt in prompts:
        results.append({"prompt": prompt, **compare_models(prompt)})
    return results
